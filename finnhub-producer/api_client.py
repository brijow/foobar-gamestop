...
class finnhub_producer:
    ...
    def query_stock_candles(self, symbol, date_from, date_to):
        ...
        df = pd.DataFrame(self.api_client.stock_candles(symbol=symbol, resolution='5', _from= from_ts, to=to_ts))
        df = df.rename(columns={'c': 'close_price', 'o':'open_price', 'h':'high-price',
                            'l':'low-price','v':'volume','t':'timestamp','s':'status'})
    ...
    def query_filling_sentiment(self, symbol, date_from, date_to):
        access_numbers_df = pd.DataFrame(self.api_client.filings(symbol=symbol, _from=date_from, to=date_to))
        ...
        for i, row in access_numbers_df.iterrows():
            ...
            if len(ac) > 0:
                try:
                    ...
                    timestamp = int(datetime.timestamp(datetime.strptime(row['filedDate'], '%Y-%m-%d %H:%M:%S')))
    ...
    def run(self):
        ...
        ts = self.query_stock_candles(symbol='GME', 
                            date_from='2021-01-01', 
                            date_to='2021-03-01')
        # print(ts)
        # ts.to_csv('stock_candle_timeseries.csv', encoding='utf-8')
        sentiment_ts = self.query_filling_sentiment(symbol='GME', 
                                                    date_from='2021-01-01', 
                                                    date_to='2021-03-01')
        # print(sentiment_ts)
